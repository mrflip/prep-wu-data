...
def extract_ids_from_a_follows_b unspliced, existing
  ...
  status = system %Q{pig -p REL=#{a_follows_b} -p RAW_REL_IDS_FILE=/azk/tmp/all_seen_users/raw_rel_ids #{File.dirname(__FILE__)}/extract_ids_from_rel.pig}
...
def extract_uid_sn_mapping unspliced, existing
  ...
  status = system %Q{pig -p TWEET_FILE=#{tweet} -p RAW_ID_SNS_FILE=/azk/tmp/all_seen_users/raw_id_sns #{File.dirname(__FILE__)}/extract_uid_sn_mapping.pig}
  exit status unless status
end
...
def assemble_twitter_user_id unspliced, existing
  ...
  status = system %Q{pig -p TW_UID=#{twitter_user_id} -p TW_SID=#{twitter_user_search_id} -p RAW_REL_IDS_FILE=/azk/tmp/all_seen_users/raw_rel_ids -p RAW_ID_SNS_FILE=/azk/tmp/all_seen_users/raw_id_sns -p MAPPING=/azk/tmp/all_seen_users/all_user_info #{File.dirname(__FILE__)}/assemble_id_mapping.pig}
  exit status unless status
end
