...
matching_tweet = FILTER tweet BY reply_to_user_id MATCHES '$USERID';
matching_search_tweet = FILTER search_tweet BY reply_to_user_id MATCHES '$USERID';
matching_user_tweet = UNION matching_tweet, matching_search_tweet;
final_tweet = FOREACH matching_user_tweet GENERATE created_at, tw_id, favorited, truncated, reply_to_user_id, reply_to_status_id, text, source, reply_to_screen_name;
rmf $OUTPUT
