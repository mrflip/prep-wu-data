...
data       = LOAD '$DATA' AS (uid:long, screen_name:chararray, created_at:long, followers:float, influx:float, outflux:float, enthusiasm:float, interesting:float, feedness:float, chattiness:float, sway:float, follow_rate:float, follow_churn:float, at_trstrank:float, fo_trstrank:float);
