...
-- load atsign graph
AtsignGraph = LOAD '$AATSIGNSSB' AS (
                  rsrc:                 chararray,
                  user_a_id:            long,
...
Out1      = FILTER FollowGraph BY user_a_id MATCHES '$USER_ID'; -- USER --> Out1
Out1Users = FOREACH Out1 GENERATE user_b_id AS user;
In1       = FILTER FollowGraph BY user_b_id MATCHES '$USER_ID'; -- USER <-- In1
In1Users  = FOREACH In1 GENERATE user_a_id AS user;

-- get atsigners and atsignees of USER
AtOut      = FILTER AtGraph BY user_a_id MATCHES '$USER_ID'; -- USER --> @AtOut
AtOutUsers = FOREACH AtOut GENERATE user_b_id AS user;
AtIn       = FILTER AtGraph BY user_b_id MATCHES '$USER_ID'; -- @USER <-- AtIn
