...
h3. Double Group
  ...
  donor_orgs   = FOREACH transactions {
    donors = SUBGROUP transactions BY donor_id; -- must be keys from GROUP read in same order; must be fewer than.
    GENERATE
      group AS foo
      COUNT(transactions) AS num_donor_org_transactions, SUM(amt) AS tot_donor_org_amt;
