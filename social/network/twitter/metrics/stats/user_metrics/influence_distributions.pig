%default TABLE 's3://s3hdfs.infinitemonkeys.info/data/sn/tw/fixd/graph/influencer_metrics_unfucked-nofriends'
...
metrics_r  = STREAM metrics_fg THROUGH `ruby /home/jacob/Programming/infochimps-data/social/network/twitter/metrics/stats/user_metrics/round_fields.rb --map` AS (followers:float, tweet_influx:float, tweet_outflux:float, enthusiasm:float, interesting:float, feedness:float, chattiness:float, sway:float, follow_rate:float, follow_churn:float, mention_trstrank:float, follow_trstrank:float);
