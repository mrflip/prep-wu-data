  ...
  if NETWORKS[index] == "twitter"
    (0..raw["results"].size-2).each do |index1|
      (index1+1..raw["results"].size-1).each do |index2|
        next if (raw["results"][index1]["Answer.Q1Url"].nil? || raw["results"][index2]["Answer.Q1Url"].nil?)
        unless raw["results"][index1]["Answer.Q1Url"].is_a?(Array)
          raw["results"][index1]["Answer.Q1Url"].delete!(" ") 
          raw["results"][index1]["Answer.Q1Url"] = raw["results"][index1]["Answer.Q1Url"].downcase.split(",")
        end
        unless raw["results"][index2]["Answer.Q1Url"].is_a?(Array)
          raw["results"][index2]["Answer.Q1Url"].delete!(" ") 
          raw["results"][index2]["Answer.Q1Url"] = raw["results"][index2]["Answer.Q1Url"].downcase.split(",")
        end
        ...
        unless intersection.empty?
          ...
          all_results[object_id]["results"][index1]["Answer.Q1Url"] = intersection
          all_results[object_id]["results"][index2]["Answer.Q1Url"] = intersection
        end
      end
    end
  else  
    (0..raw["results"].size-2).each do |index1|
      (index1+1..raw["results"].size-1).each do |index2|
        if raw["results"][index1]["Answer.Q1Url"].downcase == raw["results"][index2]["Answer.Q1Url"].downcase
...
review = File.open(WORK_DIR + NETWORKS[index] + "-" + TODAY + "-further_review.results","w")
...
all_results.each do |object_id,raw|
  if raw["match_found"]
    ...
    raw["results"].each do |result|
      review << [result["hitid"],result["hittypeid"],result["assignmentid"],result["workerid"],result["object_id"],result["website"],result["Answer.Q1Url"]].join("\t") + "\n"    
    end
  end
end