...
class JsonTweet < JsonParser
  ...
  def fix_raw!
    ...
    raw['in_reply_to_user_id']   = TwitterModelCommon.zeropad_id(raw['in_reply_to_user_id'])   unless raw['in_reply_to_user_id'].blank?
    raw['in_reply_to_status_id'] = TwitterModelCommon.zeropad_id(raw['in_reply_to_status_id']) unless raw['in_reply_to_status_id'].blank?
    scrub_hash raw, :text
  end
