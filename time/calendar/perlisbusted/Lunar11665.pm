# $Id: Lunar11665.pm 418 2007-12-04 01:20:38Z mrflipco $
#
...
use constant LUNAR_PHASE_DELTA => 10 ** -5;   # Tolerance for searches
...
sub lunar_phase_after
{
    ...
    # Lunar phase of the start date
    my $current_phase = DateTime::Util::Astro::Moon::lunar_phase($dt);
    return $dt if $dt->is_infinite;

    my $dt_moment = moment($dt);
    # Approximate fractional day# when we get to phi
    my $tau     = 
        $dt_moment + 				# $dt's day number 
	MEAN_SYNODIC_MONTH_FRAG *		# days per lunar phase degree
          mod($phi - $current_phase, 360)	# degrees from $dt to phi
    ;
    my $l       = max($dt_moment, $tau - 2);  	# don't search before $dt
    my $u       = $tau + 2;			

    # Within a 2-day window around the estimated moment,
    my $rv_moment = binary_search($l, $u,
        sub { abs($_[0] - $_[1]) <= LUNAR_PHASE_DELTA },	# find a match w/i delta tolerance
        sub { mod(DateTime::Util::Astro::Moon::lunar_phase(
            dt_from_moment($_[0])) - $phi, 360) < 180 } );	# don't wrap around
