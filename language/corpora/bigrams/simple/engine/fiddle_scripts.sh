...
dest_dir=$(readlink -f `dirname $0`/../data)
...
time cat $bigram_dir/${slug}-words.tsv
  | ruby -ne 'puts $_ unless $_.chomp =~ /[^a-zA-Z]/' \
  | ruby -ne '$_ = "^#{$_.chomp}$" ; $_.chars.to_a[0..-2].zip($_.chars.to_a[1..-1]).each{|c1,c2| puts [c1,c2].join("\t") }' \
